  ...
  async function Leaderboard(go) {
    ...
    try {
      ...
      if (me.role === 'PARENT') {
        ...
        if (children && children.length > 0) {
          ageFilter = String(children[0].age);
        }
      }
    } catch (_) { /* no auto-pick */ }
    ...
    const root = h('section', { class: 'admin-page' },
      h('div', { class: 'admin-header' },
        h('div', {},
          h('h1', {}, '🏆 Leaderboard'),
          h('p', {}, 'See how kids stack up by XP, streak, or badges.'),
        ),
        h('div', { style: 'display:flex; gap:10px;' },
          h('button', { class: 'btn btn-ghost', onclick: () => go('/dashboard') }, '← Back'),
        ),
      ),
      filterBar(),
      content,
    ...
    function renderLeaderboard(container, data) {
      ...
      if (data.leaderboard.length === 0) {
        container.appendChild(h('div', { class: 'empty-state' },
          h('div', { class: 'emoji' }, '🏆'),
          h('p', {}, 'No learners in this group yet.'),
        ));
        return;
      }
      ...
      if (rest.length > 0) {
        ...
        for (const row of rest) {
          tbody.appendChild(h('tr', { class: row.isMine ? 'my-row' : '' },
            h('td', { class: 'rank-cell' }, `#${row.rank}`),
            h('td', { class: 'avatar-cell' }, row.avatar),
            h('td', {},
              row.name,
              row.isMine ? h('span', { class: 'mine-badge' }, 'You') : null,
            ),
            h('td', {}, `Age ${row.age}`),
    ...
    function podiumCard(row, medal, isFirst, sortBy) {
      ...
      return h('div', { class: `podium-card ${isFirst ? 'first' : ''} ${row.isMine ? 'mine' : ''}` },
        ...
        h('div', { class: 'podium-meta' }, `Age ${row.age}`),
